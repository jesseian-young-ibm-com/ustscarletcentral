...
if (isset($_POST["reset_submit"])) {
    ...
    $check_email = "SELECT * FROM members WHERE ust_email='$email'";
    $result = mysqli_query($db, $check_email);

    if (mysqli_num_rows($result) == 0) {
        header("Location: ../memberlogin.php?forgot=Email does not exist");
        exit();
    } else {
        ...
        $url = "http://localhost/ustsc/memberResetPassword.php?selector=" . $selector . "&validator=" . bin2hex($token);
        ...
        header("Location: ../memberlogin.php?reset=We sent an email to reset your password");
    }
} else {
